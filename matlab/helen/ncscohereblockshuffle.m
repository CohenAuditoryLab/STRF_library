%
%function  [Cs]=ncscohereblockshuffle(Data,chan1,chan2,df,N,flag)
%
            ...
            if FLAG(chan2(l),chan1(k))
                ...
                if flag==2
                    ...
                    for n=1:size(Cxy,1)                      
                        ...
                        Cs(k,l).Block(m).C01(n,1)=CXYsort(NN);
                
                        %0.05 confidence interval
                        CXYsort=sort(Cxy(n,:));
                        NN=round(size(Cxy,2)*0.95);
                        Cs(k,l).Block(m).C05(n,1)=CXYsort(NN);
                    end
                else
                    ...
                    Cs(k,l).Block(m).C01=CXYsort(NN)*ones(size(CS(1).Cxy));
                
                    %0.05 confidence interval
                    CXYsort=sort(Cxy);
                    NN=round(length(Cxy)*0.95);
                    Cs(k,l).Block(m).C05=CXYsort(NN)*ones(size(CS(1).Cxy));
                end
            end
        end
