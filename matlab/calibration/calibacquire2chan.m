%
%function [Data]=calibacquire2chan(MicGain,PA5ATT,NB,Interface,Device,DeviceNum,SoundSelect,AcquireSelect,SpeakerSelect,ChirpGain,MicSensitivity,MicSerialNumber,FsFlag)
% 	
%   FILE NAME   : CALIB ACQUIRE 2 CHAN
% 	DESCRIPTION : Acquires input-output data for TDT Calibration. Similar
%                 to calibacuire except selects between two speaker channels
%
...
%                      5 = 195312.5     (200k)
%
%Sets the sampling rate used in the TDT .rco file. (Default, Fs=97656.25)
%
%RETURNED VARIABLES
%
...
if nargin<8 | isempty(AcquireSelect)
    AcquireSelect=0;
end
if nargin<9 | isempty(SpeakerSelect)
    SpeakerSelect=1;
end
if nargin<10 | isempty(ChirpGain)
    ChirpGain=0;
end
...
Filt1Filename=[CircuitPath(1:i-2) '\spchan1.f32'];
Filt2Filename=[CircuitPath(1:i-2) '\spchan2.f32'];
...
%Selecting Filter File Names - used for validation mode
