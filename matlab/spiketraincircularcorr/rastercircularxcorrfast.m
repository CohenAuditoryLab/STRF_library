%
%function [RData]=rastercircularxcorrfast(RASTER1,RASTER2,Fsd,Delay,NJ)
%
%   FILE NAME       : RASTER CIRCULAR CORR FAST
%   DESCRIPTION     : Shuffled circular correlation function between two
%                     rasters. "Shuffling" is perferomed betweeen RASTER1
%                     and RASTER2. 
%
%                     Computes the average correllogram between
...
%                     trials and should not be the same. Otherwise, use
%                     RASTERCIRCULARSHUFCORRFAST
%
...
F1=fft(PSTH1);
F2=fft(PSTH2);
R12=real(ifft(F1.*conj(F2)))/Fsd/T;
R12=R12/M^2;
...
%    Rdiag=real(ifft(F.*conj(F),[],2)/Fsd/T);
    ...
    for k=1:NJt        
        ...
        F1=fft(PSTH1);
        F2=fft(PSTH2);
        RJt=real(ifft(F1.*conj(F2)))/Fsd/T;
...
RData.R12Jt=R12Jt;
RData.R12set=R12set;
RData.lambda1 = mean(mean(RAS1));  % average firing rate
RData.lambda2 = mean(mean(RAS2));  % average firing rate