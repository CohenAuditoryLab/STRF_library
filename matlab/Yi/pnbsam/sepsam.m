...
function [RASonset,RASsus,Qreli]= sepsam(RASspet,Lbound,Hbound,FMAxis,stimmod,onset,num)
...
for FMindex = 1:length(FMAxis)
 ...
 for n = (RAStt.N*FMindex-RAStt.N+1):(RAStt.N*FMindex)
  ...
  for k=(RAStt.N*(FMindex-1)+1):(RAStt.N*FMindex)
   Tcyc = mod(RASspet(k).spet/RASspet(k).Fs,1/FM);
   if Lbound(FMindex)<1/FM
    i_onset=find(Tcyc < Hbound(FMindex) & Tcyc > Lbound(FMindex) );
    RASonset(k).spet = RASspet(k).spet(i_onset);
    i_sus=find(Tcyc >= Hbound(FMindex) | Tcyc < Lbound(FMindex));
    RASsus(k).spet = RASspet(k).spet(i_sus);
   else
    RASonset(k).spet = RASspet(k).spet;
    
   end  % end if Lbound<1/FM
