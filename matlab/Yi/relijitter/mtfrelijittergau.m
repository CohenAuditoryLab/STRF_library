...
% RASk=RASTER;
  ...
  T=1/FMAxis(FMi);
  betaL=[0 0 0];  % lower bound of beta
  betaU=[max(Rctr4) max(Rctr4)*1.2 T];  % upper bound of beta
...
[beta,R0,J0]=lsqcurve3optim(Tauctr4,Rctr4,T,betaL,betaU);
  
  MTFJ(FMi).DC=beta(1);
  MTFJ(FMi).lambdanoise=sqrt(beta(1)); 
  MTFJ(FMi).Rpeak=beta(2);
  MTFJ(FMi).sigmag=abs(beta(3))*1000/sqrt(2);  % sigma
  MTFJ(FMi).gaussfun=gaussfun(beta,Tauctr4,T); % 4-cycle Gaussian model
  MTFJ(FMi).gausssingle=beta(1)+beta(2)*exp(-Tauctr4.^2/(2*beta(3)^2)); % single Gaussian

  plot(Tauctr4,gaussfun(beta,Tauctr4,T));
  hold on;
  plot(Tauctr4,R.Rshuf(Nctr-4*NP:Nctr+4*NP-1),'r');
  plot(Tauctr4,beta(1),'g-');
  plot(Tauctr4, beta(1)+beta(2)*exp(-Tauctr4.^2/(2*beta(3)^2)),'k');
%   plot(Tauctr4, beta(1)+beta(2)*exp(-(Tauctr4-T).^2/(2*beta(3)^2)),'k');
%   plot(Tauctr4, beta(1)+beta(2)*exp(-(Tauctr4+T).^2/(2*beta(3)^2)),'k');
%   plot(Tauctr4, beta(1)+beta(2)*exp(-(Tauctr4-2*T).^2/(2*beta(3)^2)),'k');
%   plot(Tauctr4, beta(1)+beta(2)*exp(-(Tauctr4+2*T).^2/(2*beta(3)^2)),'k');
  xlim([-T/2 T/2]);
  hold off;
  pause(1);
  
  Rshufreli = Rctr4-beta(1);
  MTFJ(FMi).p = sqrt((sum(Rshufreli))/4/Fsd / FMAxis(FMi));   % reliability
%   MTFJ(FMi).RI = sqrt(sum(Rshufreli./MTFJ(FMi).lambda^2)/Fsd*FMAxis(FMi)); 
  MTFJ(FMi).EFF=sum(Rshufreli)/sum(Rctr4);
